...
import MESSAGE from "../../constants/messege";
...
const controller = {
    async login(req: Request, res: Response) {
        ...
        if (!bcrypt.compareSync(senha, targetUser.senha)) {
            return res.status(401).json(MESSAGE.ERROR.EMAIL_INVALID);
        }
    ...
    async recuperarSenha(req: Request, res: Response) {
        ...
        if (!savedUser) {
            logger.error(
                `[recuperarSenha] Email não encontrado email= ${email}`
            );
            return res.status(404).json("Email não encontrado");
        }
        ...
        await savedUser.save();
        logger.info(
            `[recuperarSenha] finalizando a função com a senha alterada`
        );
